#!/usr/bin/env python3
"""Run Stage 2 gates and write review-candidate evidence from a clean reconstruction."""
...
def main() -> int:
    ...
    commands = [
        ...
        [
            ...
            "src",
            "tests/stage2",
            "scripts/generate_stage2_contracts.py",
            "scripts/generate_stage2_evidence.py",
            "scripts/reconcile_stage2_records.py",
            "scripts/verify_stage2.py",
            "scripts/build_stage2_candidate_evidence.py",
        ],
        [uv, "run", "--frozen", "mypy"],
    ...
    for relative in (
        ...
        "schemas/golden/canonicalization-v1.json",
        "baseline/source/LAOS_v7.0_Complete_System(1).zip",
        "dist/laos_v8-8.0.0a2.tar.gz",
        "dist/laos_v8-8.0.0a2-py3-none-any.whl",
    ):
        path = ROOT / relative
    ...
    report = {
        ...
        "stage": 2,
        "stage_name": "Typed kernel and platform contracts",
        ...
        "typed_kernel_implemented": True,
        "privileged_v8_runtime_implemented": False,
        "v8_release_published": False,
    }
    args.output_dir.mkdir(parents=True, exist_ok=True)
    json_path = args.output_dir / "STAGE_2_CANDIDATE_EVIDENCE.json"
    ...
    (args.output_dir / "STAGE_2_CANDIDATE_EVIDENCE.md").write_bytes(("\n".join(lines) + "\n").encode("utf-8"))
